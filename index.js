...
function loadStudentsData() {
    ...
    reader.onload = function() {
        ...
        for (let i = 1; i < lines.length; i++) {
            let student = lines[i].trim().split(';'); // разделитель - точка с запятой
            if (student.length > 1) {
                studentsData[student[0]] = {
                    class: student[1], // добавляем новый столбец class
    ...
    reader.readAsText(fileInput.files[0], 'UTF-8'); // Указываем кодировку UTF-8
}
...
function displayEditTable() {
    ...
    for (let student in studentsData) {
        let row = '<tr>';
        row += '<td contenteditable="true">' + decodeURIComponent(student) + '</td>'; // декодируем и отображаем кириллицу
    ...
    // Добавляем кнопку "Добавить" под таблицей
...
function addNewStudent() {
    let table = document.getElementById('editTable');
    let newRow = table.insertRow(); // Вставляем новую строку в конец таблицы
    ...
    // Добавляем нового студента в массив данных
...
function saveData(format) {
    ...
    for (let i = 1; i < tableRows.length; i++) { // Начинаем с 1, чтобы пропустить заголовок
        ...
        let studentClass = cells[1].textContent.trim(); // Изменяем имя переменной на studentClass
...
function calculateStatistics() {
    let statistics = {};
    let classStatistics = {}; // Для хранения статистики по каждому классу и предмету
    let totalGrades = {}; // Для хранения статистики по всем ученикам и каждому предмету
